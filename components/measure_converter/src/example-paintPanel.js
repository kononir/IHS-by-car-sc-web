    ...
    _initMarkup: function (containerId) {
		...
		container.append(
			'<div id="measure-converter" class="sc-no-default-cmd"> \
				...
				<div id="input-area"> \
					<div id="input-field"> \
						<label for="convertibleMeasure" id="convertible-measure-label">Конвертируемая единица измерения</label> \
						<select size="1" id="convertibleMeasure"> \
							<option disabled selected>Единица измерения</option> \
						</select></br> \
					</div> \
					<div id="input-field"> \
						<label for="convertedMeasure" id="converted-measure-label">Конвертированная единица измерения</label> \
		...
		SCWeb.core.Server.resolveScAddr(['measure_converter'], function (keynodes) {
			$('#component').attr("sc_addr", keynodes['measure_converter']);
		});

		SCWeb.core.Server.resolveScAddr(['rrel_convertible_measure'], function (keynodes) {
			$('#convertible-measure-label').attr("sc_addr", keynodes['rrel_convertible_measure']);
		});

		SCWeb.core.Server.resolveScAddr(['rrel_converted_measure'], function (keynodes) {
			$('#converted-measure-label').attr("sc_addr", keynodes['rrel_converted_measure']);
		});
